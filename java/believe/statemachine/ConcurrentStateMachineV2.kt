...
class ConcurrentStateMachineV2<D, A> private constructor(
    private val subStateSubStateMachines: List<SubStateMachine<D, *, A>>, initialData: D
) {
    /**
     ...
     */
    val data: D = initialData.apply { updateData(this) }

    /** Attempts to transition to another state, where the transition is triggered by [action]. */
    fun transition(action: A) {
        subStateSubStateMachines.forEach { it.stateMachine.transition(action) }
        updateData(data)
    }

    private fun updateData(data: D) = subStateSubStateMachines.forEach { it.applyTo(data) }
    ...
    class Builder<D1, A>(private val initialData: D1) {
        ...
        fun build() = ConcurrentStateMachineV2(subStateSubStateMachines, initialData)
    }
